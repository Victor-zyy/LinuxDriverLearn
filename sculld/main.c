...
static ssize_t sculld_aio_read(struct kiocb *iocb, struct iov_iter *iov)
{

}
static ssize_t sculld_aio_write(struct kiocb *iocb, struct iov_iter *iov)
{
...
int sculld_init(void)
{
    ...
    sculld_create_proc(); /* Only when debugging */
#endif
	return 0; /* succeed */
